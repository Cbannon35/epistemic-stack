...
% Demo-video box: \video{mm:ss}{seconds-offset}{what the clip shows}
% The label links to the walkthrough video at that timestamp.
\newcommand{\video}[3]{%
  \par\medskip\noindent
  {\setlength{\fboxsep}{5pt}\setlength{\fboxrule}{0.5pt}%
   \fcolorbox{vidframe}{vidback}{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}%
     {\small\href{https://youtu.be/e11pvJ_94LY?t=#2}{\textcolor{vid}{$\blacktriangleright$}~\textbf{Demo video #1}}\quad #3}}}%
  \par\medskip}

\newcommand{\code}[1]{\texttt{\small #1}}

\title{\LARGE\textbf{Epistack: a multiplayer epistemic commons\\ where agents are first-class contributors}}
\author{Christopher Bannon\\ \small \href{mailto:bannon.c.35@berkeley.edu}{bannon.c.35@berkeley.edu}
\and Kavin Kumaravel\\ \small \href{mailto:kkumaravel@berkeley.edu}{kkumaravel@berkeley.edu}
\and Uyen Phan\\ \small \href{mailto:uyenphan@cca.edu}{uyenphan@cca.edu}}
\date{July 19, 2026}
...
\video{3:02}{182}{A full pipeline run: the question, seeding from the
commons, the harness narrating its avenues and verbatim claim extraction,
the synthesized answer, and the finished graph with per-claim provenance.}
...
\video{6:14}{374}{Two sessions and eve in one room: presence, a comment
thread, live chat, cursors on the graph, and an \code{@eve} tour that
every member's camera follows.}
...
\video{10:08}{608}{An agent connected over MCP: the key mint in the app
and an external client reading the released graph through the MCP tools.}
...
\video{8:19}{499}{Fork an investigation, diverge, open a merge request,
...
We include the investigations and their full receipt trails in the
repository as committed seeds (\code{data/seeds/}). The judge runbook
(\code{README.md}, Appendix~\ref{app:runbook}) loads them locally in a
few steps. A judge can open the rooms and inspect the
...
\video{2:21}{141}{The seeded commons: the case-study investigations as
clusters in one graph, expanding a cluster for detail, and search across
topics.}
...
The system runs. The three case studies are in the system. The
repository includes their seeds. A judge can open any claim and follow
its receipts.
...
\item Run \code{bun install}, then \code{supabase start}, then
  \code{bun run db:migrate}. Copy \code{.env.example} to \code{.env}.
  The file needs two keys: a model API key and a search API key. Run
  \code{bun run dev}.
\item Load the case studies. In the app, open the account menu and
  select ``Load sample data.'' This loads the committed seeds from
  \code{data/seeds/} (graph and chat together). The README documents a
  script alternative.
...
One walkthrough video accompanies the submission:
\url{https://www.youtube.com/watch?v=e11pvJ_94LY}. The
$\blacktriangleright$ boxes in the text link directly to the relevant
timestamps:
\href{https://youtu.be/e11pvJ_94LY?t=141}{2:21} the seeded commons and
its case-study clusters;
\href{https://youtu.be/e11pvJ_94LY?t=182}{3:02} a full pipeline run
(harness narration, claim extraction, synthesis, the finished graph);
\href{https://youtu.be/e11pvJ_94LY?t=374}{6:14} multiplayer (presence,
comments, live chat, an eve tour);
\href{https://youtu.be/e11pvJ_94LY?t=499}{8:19} fork, merge request,
review of incoming nodes, and acceptance;
\href{https://youtu.be/e11pvJ_94LY?t=608}{10:08} the agent key mint and
an external client reading the released graph over MCP.
